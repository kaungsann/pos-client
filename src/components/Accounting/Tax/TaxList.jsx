...
const columns = [
  ...
  { name: "TaxRate", uid: "taxrate", sortable: true },
...
export default function TaxList({ taxs }) {
  ...
  const renderCell = React.useCallback((taxs, columnKey) => {
    const cellValue = taxs[columnKey];

    switch (columnKey) {
      case "name":
        return <h3>{taxs.name}</h3>;
      case "taxrate":
        return <h3>{taxs.taxRate}</h3>;
      case "created":
