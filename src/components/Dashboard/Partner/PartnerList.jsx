...
export default function PartnerList({ partners, onDeleteSuccess }) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (
      ...
      filteredPartners = filteredPartners.filter((partner) =>
        Array.from(statusFilter).includes(partner.status)
      );
    }
  ...
  const renderCell = React.useCallback((partners, columnKey) => {
    const cellValue = partners[columnKey];

    switch (columnKey) {
      case "name":
        return <User name={cellValue}>{partners.name}</User>;
      case "email":
        return <h3>{partners.email}</h3>;
      case "phone":
        return <h3>{partners.phone ? partners.phone : "none"}</h3>;
      case "city":
        return <h3>{partners.city ? partners.city : "none"}</h3>;
      case "address":
        return <h3>{partners.address ? partners.address : "none"}</h3>;
      case "company":
        return <h3>{partners.isCompany ? "vendor" : "none"}</h3>;
      case "customer":
        return <h3>{partners.isCustomer ? "customer" : "none"}</h3>;
      case "create":
        return <h3>{format(new Date(partners.createdAt), "yyyy-MM-dd")}</h3>;
      case "status":
        return (
          <Chip
            className="capitalize"
            color={statusColorMap[partners.phone]}
            size="sm"
            variant="flat"
          >
            {cellValue}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <h2 className="text-xl font-bold my-2">Partners</h2>
